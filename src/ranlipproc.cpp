...
{
//	Rprintf("\n%s %d  %f %f", "inside init: ", dim, left[0],right[0]);
	RanLipGenerator.Init(dim,left,right);
//		Rprintf("\n%s %d  %f %f %x ", "ouside init: ", dim, left[0],right[0],&RanLipGenerator);
}
...
int	SavePartitionRanlip(const char * fname)
{
	return RanLipGenerator.SavePartition(fname);
}

// loads the hat function from the file
int	LoadPartitionRanLip(const char * fname)
{
