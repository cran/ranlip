...
extern  "C"	ranlux_state_t RLSTATE;
...
void CRanLip::RandomVec(double* p)
{
	while(Computed) {  // must be 1 to generate random variates
		...
		double d=Distribution(p);
			
		if( d> Probabilities[m_chosenElement]) //return;
		{  count_errors++; 
//		    printf("fail %f %f\n",Distribution(p),Probabilities[m_chosenElement]); 
		return;}

		if(Y <= d) {/*count_success++;*/ return;}
	}
}
...
int	CRanLip::SavePartition(const char * fname)
{
...
int	CRanLip::LoadPartition(const char * fname)
{
	FreeMem();
	...
	if(fp==NULL) return 2;
	i=fscanf(fp,"Dim,Elements,Volume %d %d %d %lf\n",&Dimension,&TotalElements, &num_partition, &Volume);
	if(i!=4) {fclose(fp); return 3;}; //corrupted file
	...
	if(Probabilities==NULL) return 4; // no memory

	for(j=0;j<TotalElements;j++) {
		i=fscanf(fp,"%lf\n",&r);
		if(i!=1) {fclose(fp); return 3;};
